...
class AdminDB:
    ...
    def assign_driver_to_booking(self, booking_id, driver_id):
        ...
        cur = conn.cursor()
        try:
            cur.execute("UPDATE bookings SET driver_id=%s, status=%s WHERE id=%s",
                       (driver_id, "Assigned", booking_id))
            cur.execute("UPDATE drivers SET driver_status=%s WHERE DID=%s", ("busy", driver_id))
            conn.commit()
        finally:
            cur.close()
            conn.close()
